...
if (!(Test-Path $env:USERPROFILE\AppData\Local\Microsoft\WindowsApps\wt.exe)) {
    ...
    winget install --id=Microsoft.WindowsTerminal -e --accept-package-agreements --accept-source-agreements cf
}
...
if (!(Test-Path $env:USERPROFILE\AppData\Local\Microsoft\WindowsApps\pwsh.exe)) {
    ...
    winget install --id Microsoft.Powershell --source winget --accept-package-agreements --accept-source-agreements cf
}
